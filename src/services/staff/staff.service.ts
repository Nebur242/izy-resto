...
class StaffService extends FirestoreService<StaffMember> {
  ...
  async createStaffMember(email: string, password: string, data: Omit<StaffMember, 'id' | 'email'>): Promise<string> {
    try {
  ...
  async updateStaffMember(id: string, data: Partial<StaffMember>): Promise<void> {
    try {
      const updateData = {
        ...data,
        updatedAt: new Date().toISOString()

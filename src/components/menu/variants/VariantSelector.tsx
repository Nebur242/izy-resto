      ...
      {selectedVariants
        .map(v => {
          return {
            ...v,
            variantCombination:
              v?.variantCombination?.length > 0
                ? v.variantCombination.filter(Boolean)
                : [],
          };
        })
        .map((variantPrice, index) => (
          <div
            key={index}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      value={
                        variantPrice.variantCombination?.length > 0
                          ? variantPrice.variantCombination[0]?.split(
                              ': '
                            )[0] || ''
                          : ''
                      }
                  ...
                  <div>
                    ...
                    <select
                      className="w-full rounded-lg border dark:border-gray-600 p-2 dark:bg-gray-700"
                      value={
                        variantPrice.variantCombination[0]?.split(': ')[1] || ''
                      }
                      onChange={e => {
                        const variantType =
                          variantPrice.variantCombination[0]?.split(': ')[0];
                        if (variantType) {

...
export function ProductDetailsModal({ item, onClose, onAddToCart }: ProductDetailsModalProps) {
  ...
  const getVariantPrice = () => {
    ...
    const variantPrice = itemWithVariants.variantPrices.find(vp => 
      JSON.stringify(vp.variantCombination.sort()) === JSON.stringify(selectedVariants.sort())
    );
  ...
  const handleAddToCart = () => {
    ...
    const productName = variantValues ? `${item.name} ${variantValues}` : item.name;
    const variantId = getVariantId();
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-4 sm:p-6 space-y-4">
            ...
            {/* Variants Selection */}
            {variantTypes && Object.entries(variantTypes).map(([type, values]) => {
                ...
                <div key={type}>
                  ...
                  <div className="flex flex-wrap gap-1 sm:gap-2">
                    {Array.from(values).map(value => {
                      ...
                      const isSelected = selectedVariants.includes(variantString);
            ...
            {getCartItem() && (
              <div className="bg-blue-50 dark:bg-blue-900/30 p-2 sm:p-2.5 rounded-lg text-blue-600 dark:text-blue-400 text-[10px] sm:text-xs font-medium text-center">
                Déjà dans le panier: {getCartItem()?.quantity} {getCartItem()?.quantity > 1 ? 'unités' : 'unité'}
              </div>
            )}
          </div>
        </div>
          ...
          <div className="flex items-center justify-between pb-5">
            <div className="flex items-center gap-3">
              ...
              <Button
                variant="secondary"
                size="sm"
                onClick={() => setQuantity(q => Math.min(item.stockQuantity, q + 1))}
